"""
Module provides a function, syllabify_word, that syllabifies a Kanien'kéha' word.
...
_CONSONANT_GROUPS: Set[str] = set()


_NASALIZED_CONSONANTS: Set[str] = set()

# Prefix groups are groups of characters that serve as common prefixes. For details,
# see README.
_PREFIX_GROUPS: Set[str] = set()
...
def _replace_semivowels_and_v(word: str) -> str:
    ...
    Where "i" is serving as a semivowel, it is replaced with "j".
    ...
    while word != "":
        char = word[0]
        if char != "i" or len(word) == 1:
            word_w_repl += char
        elif char in _VOWELS_IY:
            if word[1] in _VOWELS_AEOU:
                word_w_repl += "j"
            else:
                word_w_repl += "i"
        word = word[1:]
...
def _get_vowel_positions(word: str) -> List[int]:
    ...
    logging.debug("Vowel positions: %s", vowel_positions)
...
def _syllabify(word: str) -> List[int]:
    ...
    if len(vow_pos) == 1:
        logging.debug("### Final word syllabification: %s%s", word_stem)
        return syllable_boundaries
    ...
    for syl_bound, v_pos_1, v_pos_2 in zip(
        ...
        syllable_boundaries.append(syl_bound)
    logging.debug(
